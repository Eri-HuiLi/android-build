...
/** 网盘平台 */
enum class SharePlatform { QUARK, UC, XUNLEI, BAIDU, C139, PAN123, CTFILE }
...
object ShareLinkParser {
    ...
    // 城通网盘分享链接（单文件直链解析）：https://urlxx.ctfile.com/f/<fileID>-<seq>-<hash>?p=<提取码>
    private val cfileShareIdRegex = Regex("""[^/\s]+\.ctfile\.com/f/(\d+-\d+-\w+)""", RegexOption.IGNORE_CASE)
    private val pwdInUrlRegex = Regex("""[?&]pwd=([A-Za-z0-9]+)""")
    // 城通提取码在链接 ?p= 后（与其它盘 pwd 参数名不同）
    private val cfilePwdInUrlRegex = Regex("""[?&]p=([A-Za-z0-9]+)""")
    ...
    fun parse(text: String): ParsedShare? {
        ...
        // 城通链接：https://urlxx.ctfile.com/f/xxxx-xxx-xxx?p=xxxx（提取码在 ?p=）
        cfileShareIdRegex.find(url)?.groupValues?.getOrNull(1)?.let { sid ->
            val pwd = cfilePwdInUrlRegex.find(url)?.groupValues?.getOrNull(1)
                ?: pwdInTextRegex.find(text)?.groupValues?.getOrNull(1)
            return ParsedShare(shareId = sid, pwd = pwd, platform = SharePlatform.CTFILE)
        }
        return null
    }
}
